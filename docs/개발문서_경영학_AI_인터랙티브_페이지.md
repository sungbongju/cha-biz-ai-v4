...
→ action=navigate? → navigateToSection(tabId) 직접 호출 → scrollIntoView
→ callProxy("/v1/streaming.task", { text: ttsReply }) → HeyGen TTS 발화
→ 발화 예상 시간 후 Web Speech 재개
```
...
const SECTION_INFO = {
  ...
  ai:         { keywords: ["차별성", "복수전공", "교수", "달라", "다른 대학", "차이", "특별", ...] },
  careers:    { keywords: ["취업", "진로", "경영기획", "졸업하면", "취직", ...] },
...
**3단계 발음 최적화 (프롬프트 → 후처리 → 조사 분리)**

**① 프롬프트 규칙**: GPT에게 합성어 띄어쓰기, 영어 약어 한글화 지시
**② 후처리 강제 치환**: GPT가 규칙 미준수 시에도 route.ts에서 강제 치환
...
- heygen-proxy.js는 화이트리스트 엔드포인트만 프록시 (streaming.new/start/task/stop)
...
| 인사말 발화 완료 후 자동 음성인식 시작 | greetDuration 타이머 후 startListening() |
...
aiforalab.com
<v4 Netlify 도메인>.netlify.app  ← 배포 후 추가 필요
localhost / 127.0.0.1
```
...
2. Netlify에서 레포 연결 (sungbongjur@gmail.com 계정)
3. 환경변수 설정: `HEYGEN_API_KEY`, `OPENAI_API_KEY`
4. 백엔드 PHP API에 새 Netlify 도메인 CORS 추가
5. 카카오 개발자 콘솔에 Redirect URI 추가
...
| 03-07 | **v4 생성**: Netlify 통합 아키텍처 (iframe 제거 → livekit-client 직접 통합), Netlify Functions 3개 (heygen-token/proxy, openai-chat), route.ts 전체 포팅, HeyGen Live Avatar 전환 가이드 추가 |
